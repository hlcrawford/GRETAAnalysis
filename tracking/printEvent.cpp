...
//   printf("got type %i\n", type);
  if (type == GEB_TYPE_DECOMP)
    snprintf (str, 132, "GEB_TYPE_DECOMP      ");
  else if (type == GEB_TYPE_BGS)
    snprintf (str, 132, "GEB_TYPE_BGS         ");
  else if (type == GEB_TYPE_RAW)
    snprintf (str, 132, "GEB_TYPE_RAW         ");
  else if (type == GEB_TYPE_TRACK)
    snprintf (str, 132, "GEB_TYPE_TRACK       ");
  else if (type == GEB_TYPE_S800_RAW)
    snprintf (str, 132, "GEB_TYPE_S800_RAW    ");
  else if (type == GEB_TYPE_NSCLnonevent)
    snprintf (str, 132, "GEB_TYPE_NSCLnonevent");
  else if (type == GEB_TYPE_GT_SCALER)
    snprintf (str, 132, "GEB_TYPE_GT_SCALER   ");
  else if (type == GEB_TYPE_GT_MOD29)
    snprintf (str, 132, "GEB_TYPE_GT_MOD29    ");
  else if (type == GEB_TYPE_S800PHYSDATA)
    snprintf (str, 132, "GEB_TYPE_S800PHYSDATA");
  else if (type == GEB_TYPE_G4SIM)
    snprintf (str, 132, "GEB_TYPE_G4SIM       ");
  else if (type == GEB_TYPE_CHICO)
    snprintf (str, 132, "GEB_TYPE_CHICO       ");
  else if (type == GEB_TYPE_DGS)
    snprintf (str, 132, "GEB_TYPE_DGS         ");
  else
    snprintf (str, 132, "unknown");
//      printf("type: %s\n",str);
  ...
  for (j = 0; j < grh->ngam; j++)
    {
      ...
      fprintf (fp, "1'th hit: (%6.3f, %6.3f, %6.3f), e= %6.3f\n", grh->gr[j].x0, grh->gr[j].y0, grh->gr[j].z0, grh->gr[j].e0);
      fprintf (fp, "2'nd hit: (%6.3f, %6.3f, %6.3f), e= %6.3f\n", grh->gr[j].x1, grh->gr[j].y1, grh->gr[j].z1, grh->gr[j].e1);
    };
          ...
          for (j = 0; j < gtinp->num; j++)
            {
              ...
              r1 = gtinp->intpts[j].x * gtinp->intpts[j].x + gtinp->intpts[j].y * gtinp->intpts[j].y + gtinp->intpts[j].z * gtinp->intpts[j].z;
              r1 = sqrtf (r1);
...
//        fprintf (fp, "\n");
      ...
      fprintf (fp, "detno: %2i ", shellhit->detno[j]);
      fprintf (fp, "used: %i", shellhit->used[j]);
...
//      fprintf (fp, "\n");
  ...
  if (Clstr[iCluster].valid)
    {
      fprintf (fp, " (valid) #interactions=%2i ", Clstr[iCluster].ndet);
      fflush (fp);
      ...
      for (i = 0; i < Clstr[iCluster].ndet; i++)
        {
          ...
          fprintf (fp, "( %8.4f %8.4f %8.4f %8.4f ) ", Clstr[iCluster].intpts[i].xx, Clstr[iCluster].intpts[i].yy,
                   Clstr[iCluster].intpts[i].zz, Clstr[iCluster].intpts[i].edet);
          fprintf (fp, "order: %1i ; ", Clstr[iCluster].intpts[i].order);
          fprintf (fp, "detno: %2i ", Clstr[iCluster].intpts[i].detno);


          rr = Clstr[iCluster].intpts[i].xx * Clstr[iCluster].intpts[i].xx
            + Clstr[iCluster].intpts[i].yy * Clstr[iCluster].intpts[i].yy + Clstr[iCluster].intpts[i].zz * Clstr[iCluster].intpts[i].zz;
          rr = (float) sqrt ((double) rr);
          fprintf (fp, "r=%6.2fcm ", rr);
...
printAllClusters (char *fn, char *str, CLUSTER_INTPTS Clstr[MAXCLUSTERHITS], int *nClusters)
{
  ...
  /* the pair production events */

  fprintf (fp, "\n");
  if (npp > 0)
    fprintf (fp, "we have %i pair product event(s)\n", npp);
  else
    fprintf (fp, "we have no pair product events\n");
  for (i = 0; i < npp; i++)
    {
      fprintf (fp, "  %i/%i", i + 1, npp);
      fprintf (fp, "( %8.4f %8.4f %8.4f )", ppx[i], ppy[i], ppz[i]);
      fprintf (fp, " e %6.3f fom=%6.4f ", ppe[i] ,ppfom[i]);
      fprintf (fp, "TS= %lli",ppTS[i] );
      fprintf (fp, "\n");
      HH += ppe[i];
      KK++;
      i0++;
