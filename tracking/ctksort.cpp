...
ctksort (int iCluster, CLUSTER_INTPTS Clstr[MAXCLUSTERHITS], int *nClusters)
{
  ...
  /* 200 keV or so. (really rough rule) */

  /* bubble sort on segment energies */
  /* so we start with most likely order */
      ...
      if (Clstr[iCluster].intpts[i].edet < Clstr[iCluster].intpts[j].edet)
        {
          ...
          itmp = Clstr[iCluster].intpts[i].shellHitPos;
          Clstr[iCluster].intpts[i].shellHitPos = Clstr[iCluster].intpts[j].shellHitPos;
          Clstr[iCluster].intpts[j].shellHitPos = itmp;

          itmp = Clstr[iCluster].intpts[i].order;
          Clstr[iCluster].intpts[i].order = Clstr[iCluster].intpts[j].order;
          Clstr[iCluster].intpts[j].order = itmp;

          itmp = Clstr[iCluster].intpts[i].detno;
          Clstr[iCluster].intpts[i].detno = Clstr[iCluster].intpts[j].detno;
          Clstr[iCluster].intpts[j].detno = itmp;

          ltmp = Clstr[iCluster].intpts[i].timestamp;
          Clstr[iCluster].intpts[i].timestamp = Clstr[iCluster].intpts[j].timestamp;
          Clstr[iCluster].intpts[j].timestamp = ltmp;
