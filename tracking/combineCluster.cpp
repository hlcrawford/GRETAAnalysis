                ...
                int *nClusters, float target_pos[3])
{
  ...
  int nCandidates = 0;
  int nbase, best, tryClstr, bestCombSoFar = -1;
  ...
  Clstr[best].valid = 0;
  tryClstr = best + 1;               /* trial cluster position */

#if (DEBUG)
  fprintf (stderr,"best=%i, tryClstr=%i\n", best, tryClstr);
  fflush (stdout);
#endif

  /* copy our focus cluster into the trial cluster */

  memcpy ((void *) &Clstr[tryClstr], (const char *) &Clstr[focusCluster], sizeof (struct CLUSTER_INTPTS));
            ...
            if (dist < Pars.combineMaxDist)
              {
                ...
                Clstr[tryClstr].esum = esum;
                for (j = 0; j < Clstr[i].ndet; j++)
                  {
                    Clstr[tryClstr].ndet++;
                    Clstr[tryClstr].esum += Clstr[i].intpts[j].edet;
                    Clstr[tryClstr].intpts[j + nbase].edet = Clstr[i].intpts[j].edet;
                    Clstr[tryClstr].intpts[j + nbase].xx = Clstr[i].intpts[j].xx;
                    Clstr[tryClstr].intpts[j + nbase].yy = Clstr[i].intpts[j].yy;
                    Clstr[tryClstr].intpts[j + nbase].zz = Clstr[i].intpts[j].zz;
                    Clstr[tryClstr].intpts[j + nbase].timestamp = Clstr[i].intpts[j].timestamp;
                    Clstr[tryClstr].intpts[j + nbase].detno = Clstr[i].intpts[j].detno;
#if SIMULATED
                    Clstr[tryClstr].intpts[j + nbase].origPos = Clstr[i].intpts[j].origPos;
                    Clstr[tryClstr].intpts[j + nbase].esumOrig = Clstr[i].intpts[j].esumOrig;
#endif
                  };
                ...
                if (Clstr[tryClstr].ndet < MAXNOSEG)
                  {
#if (DEBUG)
                ...
                if (Clstr[tryClstr].fom < bestFomSoFar)
                  {
                    if (Pars.nprint > 0)
                      fprintf (stderr,"best candicate so far, with FOM=%f\n", Clstr[tryClstr].fom);
                    nCandidates++;
                    bestFomSoFar = Clstr[tryClstr].fom;
                    bestCombSoFar = i;
                    assert (*nClusters < MAXCLUSTERHITS);
                    assert (bestCombSoFar < *nClusters);
                    memcpy ((void *) &Clstr[best], (const char *) &Clstr[tryClstr], sizeof (struct CLUSTER_INTPTS));
                  }
