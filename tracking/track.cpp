...
/* npp / ppe / ... live in GretaTrackingWorkspace (see greta_tracking_std.hpp). */
...
//              gtinp->intpts[j].e = gtinp->intpts[j].e * Pars.SEGcal_gain[i1][gtinp->intpts[j].seg] + Pars.SEGcal_offset[i1][gtinp->intpts[j].seg];
          ...
          if (Pars.useCCEnergy)
            {
              ...
              /* only works for low count rates */
	      
	      //	      for (j = 0; j < gtinp->num; j++)
	      //		printf("original %f\n", gtinp->intpts[j].e);

	      //	      printf("gtinp->tot_e %f\n", gtinp->tot_e);
          ...
          if (Pars.enabled[i1])
            {
              ...
              /* check for negative energy hits */
...
findClusters (SHELLHIT *shellhit)
{
  ...
  /* only use hits that pairprod2 did not claim */

  for (i = 0; i < shellhit->nhit; i++)
    if (shellhit->used[i]==0)
    for (j = 0; j < shellhit->nhit; j++)
    if (shellhit->used[j]==0)
      if (i != j)
        {
          ...
          if (shellhit->relAng[j][i] < Pars.alpha[shellhit->nhit])
            {
              ...
              else if (shellhit->ClusterNumber[i] >= 0 && shellhit->ClusterNumber[j] >= 0 && shellhit->ClusterNumber[i] != shellhit->ClusterNumber[j])
                {
  ...
  while (pc != NULL)
    {
      ...
      if (str[0] == 35)
      ...
      else if ((p = strstr (str, "echo")) != NULL)
        {
          echo = 1;
      ...
      else if ((p = strstr (str, "weightedfom")) != NULL)
        {
          CheckNoArgs (nret, 2, str);
          nret = sscanf (str, "%s %f", str1, &Pars.weightedpower);
          Pars.weightedfom = 1;
          fprintf (stderr, "will use the weighted FOM measure, with power: %f\n", Pars.weightedpower);
        }
      ...
      else if ((p = strstr (str, "CCcal")) != NULL)
        {
          ...
          if (fp1 == NULL)
            ...
            {
              fprintf (stderr, "reading CCcal file \"%s\"\n", str2);
              nn = 0;
              ...
              fprintf (stderr, "done, read %i calibration coefficients \n", nn);
              fclose (fp1);
            };
        }
        ...
        {
          printf ("This option is nolonger available, please remove\n");
          exit (0);
        }
      ...
      else if ((p = strstr (str, "splitclusters2")) != NULL)
        {
          nret =
            sscanf (str, "%s %f %f %i %i %i %f", str2, &Pars.splitclusters_kickoutfom2, &Pars.splitclusters_thresh2,
                    &Pars.splitclusters_minn2, &Pars.splitclusters_maxn2, &Pars.splitclusters_maxtry2, &Pars.goodImproveFraction);
      ...
      else if ((p = strstr (str, "trackDataStream")) != NULL)
        {
          ...
          if (Pars.trackDataStream < 0)
            ...
            {
              printf ("it appears that the tracked output file is already open. ignoring the trackDataStream %s line in the chat file\n", str2);
            }
#endif
        }
...
//          fprintf (stderr,"Pars.alpha=%f rad %f deg\n", Pars.alpha, Pars.alpha / RAD2DEG);
        ...
        } else if ((p = strstr (str, "ppfind")) != NULL)
        {
         nret = sscanf (str, "%s", str1);
         CheckNoArgs (nret, 1, str);
         Pars.ppfind=1;
         fprintf(stderr, "will search for and process pair production events\n");
        } else if ((p = strstr (str, "ppemin")) != NULL)
        {
         nret = sscanf (str, "%s %f", str1, &Pars.ppemin);
         CheckNoArgs (nret, 2, str);
        } else if ((p = strstr (str, "ppde")) != NULL)
        {
         nret = sscanf (str, "%s %f", str1, &Pars.ppde);
         CheckNoArgs (nret, 2, str);
        } else if ((p = strstr (str, "ppeflash")) != NULL)
        {
         nret = sscanf (str, "%s %f", str1, &Pars.ppeflash);
         CheckNoArgs (nret, 2, str);
        } else if ((p = strstr (str, "ppmaxdist")) != NULL)
        {
         nret = sscanf (str, "%s %f", str1, &Pars.ppmaxdist);
         CheckNoArgs (nret, 2, str);
        } else if ((p = strstr (str, "ppdflash")) != NULL)
        {
         nret = sscanf (str, "%s %f", str1, &Pars.ppdflash);
         CheckNoArgs (nret, 2, str);
        }
      else
        {
      ...
      if (ptgd->type == GEB_TYPE_DECOMP)
        {
          ...
          if (Pars.nprint > 0)
            for (j = 0; j < gtinp->num; j++)
              {
                fprintf (stderr, "--> %6.2f %6.2f %6.2f, %8.2f; ", gtinp->intpts[j].x, gtinp->intpts[j].y, gtinp->intpts[j].z, gtinp->intpts[j].e);
              ...
              for (j = 0; j < gtinp->num; j++)
                {
                  if (seghit[gtinp->intpts[j].seg] == 2 && fp != NULL)
                    {
                      fprintf (fp, "%6.2f %6.2f %6.2f, %8.2f; ", gtinp->intpts[j].x, gtinp->intpts[j].y, gtinp->intpts[j].z, gtinp->intpts[j].e);
            ...
            CLUSTER_INTPTS Clstr[MAXCLUSTERHITS], int *nClusters /* tracking outout */ )
{
  ...
  int i, j, k, st, numCurrentClusters, i1, itno, ii;
  ...
  int detNo[MAXDETPOS];
  static int nML = 0;
  int findGlobalHitPositions_GT (TRACK_STRUCT *, STAT *);
  int findGlobalHitPositions_AGATA (TRACK_STRUCT *, STAT *);

  /* prototype */

  int findpairprod2 (SHELLHIT *, STAT *);
...
//trapbad ("tracEvent",track);
  ...
  if (Pars.nprint > 0)
    {
      snprintf (str, sizeof (str), "A,nrrawEvent_%3.3i.list", ctkStat->nTrackCalled);
      fprintf (stderr, "%s\n", str);
      fp = fopen (str, "w");
...
//  printf("findGlobalHitPositions_GT\n"); fflush(stdout);
  ...
  if (Pars.nprint > 0)
    {
      snprintf (str, sizeof (str), "B,rawEvent_%3.3i.list", ctkStat->nTrackCalled);
      fprintf (stderr, "%s\n", str);
      fp = fopen (str, "w");
      printEvent (fp, ctkStat->nTrackCalled, track);
      fclose (fp);
...
//  printf("rackUnpack\n"); fflush(stdout);
  ...
  /* reset used in shell hit */

  for (j = 0; j < shellhit->nhit; j++)
    shellhit->used[j]=0;

  /* detect/process pairprod events */

  npp=0;
  if(Pars.ppfind)
    st = findpairprod2 (shellhit, ctkStat);
...
//  printf("findClusters\n"); fflush(stdout);
  ...
  if (GretaTrackDbgIsVerbose())
    GretaTrackDbgPrintShellHits("after findClusters", shellhit);
#endif
  ...
  if (Pars.nprint > 0)
    {
      snprintf (str, sizeof (str), "C,shellhit_%3.3i.list", ctkStat->nTrackCalled);
      fprintf (stderr, "%s\n", str);
      fp = fopen (str, "w");
...
#if(0)
  /* write out the shellhit in simple binary format */
  /* for Sven and Domenic */

  st = fwrite ((char *) &shellhit->nhit, 1, sizeof (int), fpsven);
  for (j = 0; j < shellhit->nhit; j++)
    {
      st = fwrite ((char *) &shellhit->XX[j], 1, sizeof (int), fpsven);
      st = fwrite ((char *) &shellhit->YY[j], 1, sizeof (int), fpsven);
      st = fwrite ((char *) &shellhit->ZZ[j], 1, sizeof (int), fpsven);
      st = fwrite ((char *) &shellhit->edet[j], 1, sizeof (float), fpsven);
      st = fwrite ((char *) &shellhit->timestamp, 1, sizeof (long long int), fpsven);
    };
...
#ifdef GRETA_TRACK_DEBUG
  if (GretaTrackDbgIsVerbose())
    GretaTrackDbgPrintClusters("after transferClusters", *nClusters, Clstr);
#endif

  if (Pars.nprint > 0)
    {
      snprintf (str, sizeof (str), "D,originalCluster_%3.3i.list", ctkStat->nTrackCalled);
      fprintf (stderr, "%s\n", str);
...
//  printf("ctksort\n"); fflush(stdout);
  ...
  if (Pars.nprint > 0)
    {
      snprintf (str, sizeof (str), "E,originalCluster_%3.3i.list", ctkStat->nTrackCalled);
      fprintf (stderr, "%s\n", str);
      printAllClusters (str, str, Clstr, nClusters);
    };
...
//                assert (Clstr[iCluster].fom < 150);
  ...
  if (Pars.nprint > 0)
    {
      snprintf (str, sizeof (str), "G,firstTrack_%3.3i.list", ctkStat->nTrackCalled);
      fprintf (stderr, "%s\n", str);
      printAllClusters (str, str, Clstr, nClusters);
    };
  ...
  for (itno = 0; itno < Pars.itterations; itno++)
    {
      ...
      if (Pars.recluster1)
        {
          ...
          if (Pars.nprint > 0)
            fprintf (stderr, "track:trying to split untracked monster clusters using reCluster at ev # %lli\n", ctkStat->nEvents);
          ...
          if (Pars.nprint > 0 && success > 0)
            {
              snprintf (str, sizeof (str), "H,reclustered_%3.3i.list", ctkStat->nTrackCalled);
              fprintf (stderr, "%s\n", str);
              printAllClusters (str, str, Clstr, nClusters);
            };
        };
      ...
      if (Pars.splitclusters1)
        {
              ...
              if (!Clstr[i].tracked)
                {
                  ...
                  st =
                    splitCluster (i, ctkStat->nTrackCalled, ctkStat, Clstr, nClusters, target_pos,
                                  Pars.untracked_fom_kickout, Pars.splitclusters_maxtry1, Pars.splitclusters_kickoutfom1);
                  i1 += ctkStat->TrackingCalls;
          ...
          if (Pars.nprint > 0 && success)
            {
              snprintf (str, sizeof (str), "I,splitUntracked_%3.3i.list", ctkStat->nTrackCalled);
              fprintf (stderr, "%s\n", str);
              printAllClusters (str, str, Clstr, nClusters);
            };
      ...
      if (Pars.combineclusters)
        {
          ...
          if (Pars.nprint > 0 && success)
            {
              snprintf (str, sizeof (str), "J,combinedEvents_%3.3i.list", ctkStat->nTrackCalled);
              fprintf (stderr, "%s\n", str);
              printAllClusters (str, str, Clstr, nClusters);
            };
      ...
      if (Pars.recluster2)
        {
                ...
                if (Clstr[i].ndet >= Pars.recluster_minn2)
                  {
                    ...
                    st = reCluster (i, ctkStat->nTrackCalled, ctkStat, Clstr, nClusters, target_pos,
                                    shellhit, Pars.recluster_reduxfactor2, Pars.recluster_maxtry2, Pars.recluster_kickoutfom2);
                    i1 += ctkStat->TrackingCalls;
          ...
          if (Pars.nprint > 0 && *nClusters > numCurrentClusters)
            {
              snprintf (str, sizeof (str), "L,reclusterTooBig_%3.3i.list", ctkStat->nTrackCalled);
              fprintf (stderr, "%s\n", str);
              printAllClusters (str, str, Clstr, nClusters);
            };
      ...
      if (Pars.splitclusters2)
        {
          ...
          if (Pars.nprint > 0 && success)
            {
              snprintf (str, sizeof (str), "N,splitTooBig_%3.3i.list", ctkStat->nTrackCalled);
              fprintf (stderr, "%s\n", str);
              printAllClusters (str, str, Clstr, nClusters);
            };
      ...
      if (Pars.matchmaker)
        {
          ...
          if (Pars.nprint > 0 && success)
            {
              snprintf (str, sizeof (str), "K,matchMakerEvents_%3.3i.list", ctkStat->nTrackCalled);
              fprintf (stderr, "%s\n", str);
              printAllClusters (str, str, Clstr, nClusters);
            };
      ...
      for (i = 0; i < *nClusters; i++)
        if (Clstr[i].valid)
          if (Clstr[i].ndet <= 0)
            {
              snprintf (str, sizeof (str), "Z,final_%3.3i.list", ctkStat->nTrackCalled);
...
#if(0)
  /* special print of data for Sven and Domenic */

//   if ( nML < 500 && *nClusters == 1 && (fabsf(Clstr[0].esum- 2.506) <= 0.002) )
  if (nML < 50000)
    {
      snprintf (str, sizeof (str), "filtered_%3.3i.list", nML);
      fprintf (stderr, "%s %i %f\n", str, *nClusters, Clstr[0].esum);
      printAllClusters (str, str, Clstr, nClusters);
      nML++;
    };
#endif
...
#if(0)
  /* special print of strange fom value */

  i1 = 0;
  for (i = 0; i < *nClusters; i++)
    if (Clstr[i].valid)
      if (Clstr[i].tracked)
        if (Clstr[i].fom > 1.80 && Clstr[i].fom < 1.81)
          i1++;
  if (i1 > 0)
    {
      snprintf (str, sizeof (str), "X,special_%5.5i.list", ctkStat->nTrackCalled);
      fprintf (stderr, "%s\n", str);
      printAllClusters (str, str, Clstr, nClusters);
    };
#endif
...
#ifdef GRETA_TRACK_DEBUG
  if (GretaTrackDbgIsVerbose())
    GretaTrackDbgPrintClusters("trackEvent exit (final clusters)", *nClusters, Clstr);
#endif
