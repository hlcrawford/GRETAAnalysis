...
#if(0)
          /* SPECIAL: fixup error GODDESS Jun 2021 mistake */

          if (holeNum==27) holeNum=6;
          if (holeNum==12) holeNum=9;
#endif
...
//          crystalNumber = gtinp->crystal_id % 4;
	      ...
	      if (holeNum==10 || holeNum==23) {
		//	printf("holeNum, x, y, z = %d %f %f %f\n", holeNum, gtinp->intpts[j].x, gtinp->intpts[j].y, gtinp->intpts[j].z);
	      }
              ...
              if (Pars.nprint > 0)
                {
                  ...
                  fprintf (stderr, "world   coords: x,y,z=%9.4f,%9.4f,%9.4f\n", gtinp->intpts[j].x, gtinp->intpts[j].y,
                           gtinp->intpts[j].z);
                  fprintf (stderr, "xyzoffset: x,y,z=%9.4f,%9.4f,%9.4f\n",
                           Pars.xyzoffset[holeNum * 4 + crystalNumber][0],
                           Pars.xyzoffset[holeNum * 4 + crystalNumber][1],
                           Pars.xyzoffset[holeNum * 4 + crystalNumber][2]);
                }
            };
