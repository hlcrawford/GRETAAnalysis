...
//      if (fp != NULL) fclose(fp);
      snprintf (str, sizeof (str), "F,track_%3.3i.list", ctkStat->nTrackCalled);
      fp = fopen (str, "w");
      assert (fp != NULL);
  ...
  for (curPerm = 0; curPerm < Pars.nperm[Clstr[iCluster].ndet]; curPerm++)
    {
      ...
      if (Pars.weightedfom)
        {
          /* find weights for FOM */

          r1=0;
          for (k = 0; k < (Clstr[iCluster].ndet-1); k++)
            {
#if(1)
            ww[k] = Clstr[iCluster].intpts[k].edet / Clstr[iCluster].esum;
            ww[k] += Clstr[iCluster].intpts[k+1].edet / Clstr[iCluster].esum;
            ww[k] /= 2;
#endif
#if(0)
            if (Clstr[iCluster].intpts[k].edet< Clstr[iCluster].intpts[k+1].edet)
              ww[k] = Clstr[iCluster].intpts[k].edet / Clstr[iCluster].esum;
            else
              ww[k] = Clstr[iCluster].intpts[k+1].edet / Clstr[iCluster].esum;
#endif
            ww[k] = powf(ww[k],Pars.weightedpower);
            r1+=ww[k];
            };
          ...
          if (Pars.nprint > 0)
            {
              fprintf (stderr, "xx Clstr[%i].esum=%f ", iCluster, Clstr[iCluster].esum);
              fprintf (stderr, "ndet=%i\n", Clstr[iCluster].ndet);
              for (k = 0; k < Clstr[iCluster].ndet; k++)
                {
                  fprintf (stderr, "xx    Clstr[%i].intpts[%i].xx=%f ", iCluster, k, Clstr[iCluster].intpts[k].xx);
                  fprintf (stderr, "yy=%f ", Clstr[iCluster].intpts[k].yy);
                  fprintf (stderr, "zz= %f ", Clstr[iCluster].intpts[k].zz);
                  fprintf (stderr, "edet=%f ", Clstr[iCluster].intpts[k].edet);
                  fprintf (stderr, "ww=%f\n", ww[k]);
                };
            };
        };
...
//        printf("b: added penalty %f, %f, %f\n", pfac,Pars.badTheoAngPenalty, pfac*Pars.badTheoAngPenalty);
          ...
          if (Pars.nprint > 0)
            {
              fprintf (fp, "%8.4f %8.4f %8.4f; ", Clstr[iCluster].intpts[indx[k]].xx,
                       Clstr[iCluster].intpts[indx[k]].yy, Clstr[iCluster].intpts[indx[k]].zz);
              fprintf (fp, "(%6.4f); ", Clstr[iCluster].intpts[indx[k]].edet);
              fprintf (fp, "th/cth= %6.4f/%6.4f; ", th, thc);
              r1 = sqrtf (fom) / (Clstr[iCluster].ndet - 1);
              fprintf (fp, "sum square FOM now %9.6f (%9.6f), ", fom, r1);
              fprintf (fp, "eg now %5.3f; ", eg);
              if(pfac>0)
                fprintf (fp, "pfac=%5.3f (penalty %5.3f)", pfac, pfac * Pars.badTheoAngPenalty);
              fprintf (fp, "\n");
            };
#endif
