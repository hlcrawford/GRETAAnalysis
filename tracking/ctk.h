...
typedef struct STAT
{
  ...
  int crystal_hits[200];

  int num511[30];
  int num1022[30];
  int ppstat[30];

  long long int flash_stat[30];
...
int matchMaker (int, int, int, STAT * ctkStat, CLUSTER_INTPTS Clstr[MAXCLUSTERHITS], int *nClusters,
                float target_pos[3]);
int readChatFile (char *name);
int setupTrack (struct tms *timesThen, STAT * ctkStat, SHELLHIT * shellhit);
